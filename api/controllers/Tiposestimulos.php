    ...
    try {
        ...
        if (isset($token_actual->id)) {
            ...
            if ($permiso_escritura == "ok") {
                $phql = 'SELECT p.id,p.nombre,up.id AS checked FROM Tiposestimulos AS p LEFT JOIN Usuariosareas AS up ON p.id = up.area AND up.usuario=' . $id . ' WHERE p.active = true ORDER BY p.nombre';
...
$app->get('/all', function () use ($app) {
    try {
        ...
        if (isset($token_actual->id)) {
            ...
            $sqlTot = "SELECT count(*) as total FROM Tiposestimulos AS a";
            $sqlRec = "SELECT " . $columns[0] . " , concat('<button type=\"button\" class=\"btn btn-warning\" onclick=\"form_edit(',a.id,')\"><span class=\"glyphicon glyphicon-edit\"></span></button><button type=\"button\" class=\"btn btn-danger\" onclick=\"form_del(',a.id,')\"><span class=\"glyphicon glyphicon-remove\"></span></button>') as acciones FROM Tiposestimulos AS a";
    ...
    try {
        ...
        if (isset($token_actual->id)) {
            ...
            if ($permiso_escritura == "ok") {
                ...
                $area = Tiposestimulos::findFirst(json_decode($id));
...
$app->delete('/delete/{id:[0-9]+}', function ($id) use ($app, $config) {
    try {
        ...
        if (isset($token_actual->id)) {
            ...
            if ($permiso_escritura == "ok") {
                // Consultar el usuario que se esta editando
                $user = Tiposestimulos::findFirst(json_decode($id));
                $user->active = false;
...
$app->get('/search/{id:[0-9]+}', function ($id) use ($app) {
    try {
        ...
        //Si el token existe y esta activo entra a realizar la tabla
        if (isset($token_actual->id)) {
            $area = Tiposestimulos::findFirst($id);
            if (isset($area->id)) {
