    ...
    try {
        ...
        if (isset($token_actual->id)) {
            ...
            if ($permiso_escritura == "ok") {
                $phql = 'SELECT p.id,p.nombre,up.id AS checked FROM Estados AS p LEFT JOIN Usuariosareas AS up ON p.id = up.area AND up.usuario=' . $id . ' WHERE p.active = true ORDER BY p.nombre';
...
$app->get('/all', function () use ($app) {
    try {
        ...
        if (isset($token_actual->id)) {
            ...
            $sqlTot = "SELECT count(*) as total FROM Estados AS a";
            $sqlRec = "SELECT " . $columns[0] . " , " . $columns[1] . " , concat('<button type=\"button\" class=\"btn btn-warning\" onclick=\"form_edit(',a.id,')\"><span class=\"glyphicon glyphicon-edit\"></span></button><button type=\"button\" class=\"btn btn-danger\" onclick=\"form_del(',a.id,')\"><span class=\"glyphicon glyphicon-remove\"></span></button>') as acciones FROM Estados AS a";
    ...
    try {
        ...
        if (isset($token_actual->id)) {
            ...
            if ($permiso_escritura == "ok") {
                ...
                $area = Estados::findFirst(json_decode($id));
...
$app->delete('/delete/{id:[0-9]+}', function ($id) use ($app, $config) {
    try {
        ...
        if (isset($token_actual->id)) {
            ...
            if ($permiso_escritura == "ok") {
                // Consultar el usuario que se esta editando
                $user = Estados::findFirst(json_decode($id));
                $user->active = false;
